...
// Run the PostCage sendmail replacement.
func Run() {
	...
	// if run via `postcage sendmail ...` then remove `sendmail` from "recipients"
...
func HelpTemplate(args []string) string {
	return fmt.Sprintf(`A sendmail command replacement for PostCage (%s)

Usage: %s [flags] [recipients] < message

See: https://github.com/trinhminhtriet/postcage
