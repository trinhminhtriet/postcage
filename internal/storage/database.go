...
func InitDB() error {
	...
	if p == "" {
		...
		p = fmt.Sprintf("%s-%d.db", path.Join(os.TempDir(), "postcage"), time.Now().UnixNano())
...
func Close() {
	// on a fatal exit (eg: ports blocked), allow PostCage to run migration tasks before closing the DB
